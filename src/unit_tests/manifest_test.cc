...
TEST_F(ManifestTest, MetadataElement_Valid) {
  ...
  ASSERT_EQ(meta_data_map["key1"], "");
  ASSERT_EQ(meta_data_map["key2"], "value2");
}
...
TEST_F(ManifestTest, MetadataElement_DuplicateKey) {
  ...
  ASSERT_EQ(meta_data_map.size(), 2);
  ASSERT_EQ(meta_data_map["key1"], "key1value");
  ASSERT_EQ(meta_data_map["key2"], "key2value");
}
