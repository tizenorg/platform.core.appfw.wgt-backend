...
common_installer::Step::Status StepGenerateXml::process() {
  ...
  for (auto& appwidget : backend_data->appwidgets.get().app_widgets()) {
    ...
    for (auto& size : appwidget.content_size) {
      ...
      // TODO(t.iwanek): where preview icons are copied to this location?
      // does preview icon has to be .png?
      std::string icon_name = appwidget.id + "." + type + "." + "preview.png";
      bf::path preview_icon =
          context_->pkg_path.get() / kSharedData / icon_name;
