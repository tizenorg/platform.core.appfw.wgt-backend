...
bool StepParse::FillManifestX(manifest_x* manifest) {
  return FillIconPaths(manifest) &&
         FillMainApplicationInfo(manifest) &&
         FillWidgetInfo(manifest) &&
         FillInstallationInfo(manifest) &&
         FillPrivileges(manifest) &&
         FillAppControl(manifest) &&
         FillCategories(manifest) &&
         ...
         FillServiceApplicationInfo(manifest) &&
         FillWidgetApplicationInfo(manifest) &&
         FillBackgroundCategoryInfo(manifest) &&
         FillExtraManifestInfo(manifest);
}
