...
common_installer::Step::Status StepCheckExtensionPrivileges::process() {
  ...
  for (auto it = xmlFiles.begin(); it != xmlFiles.end(); ++it){
    ...
    std::vector<std::string> list = GetExtensionPrivilegeList(*it);
...
std::string StepCheckExtensionPrivileges::GetExtensionPath(){
  ...
  if (0 == uname(&u)) {
    ...
    LOG(ERROR)<<"machine architecture : " << machine;
    if (!machine.empty()) {
      if (machine == kArchArmv7l) app_ext_config_pattern.append(kArchArmv7l);
      else if (machine == kArchI586) app_ext_config_pattern.append(kArchI586);
      else app_ext_config_pattern.append(kArchDefault);
    } else {
...
bool StepCheckExtensionPrivileges::CheckPriviligeLevel(std::set<std::string> priv_set){
  ...
  if (!common_installer::ValidatePrivilegeLevel(
         context_->privilege_level.get(), false, context_->manifest_data.get()->api_version, privileges, &error_message)) {
